...
| Name                          | URL |
|-------------------------------|-----|
| Huggingface Space (optional)  | https://huggingface.co/spaces/nishawarschonvergeben/prideandprejudice --> es braucht einfach meeega lange (knapp. 230 sekunden für die generierung von Antwort) |
...
- Scenes with potentially ambiguous retrieval (e.g. multiple proposals, timeline confusion)
- Unanswerable questions where hallucinations might occur (e.g. “second ball” question)
...
4. **Manual Expansion** (handcrafted ideal expansion by MEEEE (not an expert))
5. **LLM Expansion** (automatic query rewriting via LLM)

## Evaluation Procedure

For each model output, I manually assessed the following aspects:

- **Accuracy** (whether the answer correctly reflects the book content)
- **Depth** (level of detail and nuance)
- **Hallucination** (whether invented or incorrect content was introduced)
- **Proper handling of unanswerable questions** ("cannot answer from context")
// in the future I would like to test context fit as well --> with the used chunks

The evaluation was fully manual and based on careful cross-checking with the original book text.

# Results

| Model/Method | Accuracy | Precision | Recall |
|---------------|----------|-----------|--------|
| Retrieved Chunks with config XYZ | *(not applicable for answer evaluation)* | - | - |
| Generated Answer: Baseline | 2/5 (40%) | Medium: correct when answered, but prone to hallucination in some cases | Medium |
| Generated Answer: Rewrite 1 | 3/5 (60%) | High precision, no hallucinations | Medium |
| Generated Answer: Rewrite 2 | 2/5 (40%) | Very conservative, often refused to answer even when context was available | Low |
| Generated Answer: Manual Expansion | 4/5 (80%) | Highest precision, closest to book content, no hallucinations | High |
| Generated Answer: LLM Expansion | 4/5 (80%) | Very high semantic quality, minor mixing of timelines occasionally | High |

# Notes

- **Accuracy**: Correct answers / 5 total questions.
- **Precision**: How often the model stays fully correct when it decides to answer.
- **Recall**: Ability to answer as many questions correctly as possible (even with limited context).
